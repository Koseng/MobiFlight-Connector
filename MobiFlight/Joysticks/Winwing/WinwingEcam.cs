...
namespace MobiFlight.Joysticks.Winwing
{
    internal class WinwingEcam : Joystick
    {
        ...
        {
            Log.Instance.log($"WinwingEcam - New WinwingEcam ProductId={productId.ToString("X")}", LogSeverity.Debug);
            ...
            {
                LcdDevices.Add(new LcdDisplay() { Name = displayName });
            }
            foreach (string ledName in ledNames)
            {
                LedDevices.Add(new JoystickOutputDevice() { Label = ledName, Name = ledName }.ToListItem());         
            }
        }
