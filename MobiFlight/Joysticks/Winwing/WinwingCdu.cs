...
{
    internal class WinwingCdu : WinwingBaseController
    {
        private const int COLUMNS = 24;
        private const int ROWS = 14;
        private const int CELLS = COLUMNS * ROWS;

        public WinwingCdu(SharpDX.DirectInput.Joystick joystick, JoystickDefinition def, int productId, WebSocketServer server) : base(joystick, def, productId, server)
        {
